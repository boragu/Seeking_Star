...
export function JourneyFacts({ destination }: { destination: Destination }) {
  const calmDisplay = destination.calm !== null
    ? `${destination.calm}점`
    : (destination.concentrationRate !== null
        ? `${Math.round(100 - destination.concentrationRate)}점`
        : "여유 (비과밀)");

  const facts = [
    { icon: TrendDown, label: "오늘의 여유", value: calmDisplay },
