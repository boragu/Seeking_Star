...
[작성 지침]
당신은 천체사진 전문가입니다. 위의 실시간 천문 조건(월령, 골든타임, 광공해)을 정확히 반영하여 답변을 작성하세요.

출력 형식 규칙:
1. **첫 문단 (분석 근거 요약)**: 반드시 맨 첫 줄에 '**[분석 근거]** ${destination.name}의 ${planner.date} 관측 환경은 ${moon.phaseName}(광도율 ${moon.illumination}%, ${moon.interferenceLabel})이며, 박명 시각(${astro.optimalTimeWindow}) 및 광공해 수준을 바탕으로 산출된 촬영값입니다.' 형식으로 분석 근거를 1~2문장으로 먼저 명시하세요.
2. **이후 문단 (촬영 세팅값 및 가이드)**: 초보자도 현장에서 바로 따라 할 수 있는 구체적인 수치(ISO, 셔터스피드, 초점 등)와 핵심 촬영 꿀팁을 3~4개의 정갈한 불릿 포인트(• 기호)로 작성하세요.
3. 불필요한 인사말이나 서론/결론은 생략하세요.`;
    ...
    const res = await fetch("/api/ai/guide", {
      ...
      body: JSON.stringify({
        messages: [
          {
            role: "system",
            content: "당신은 천체사진 및 야간 촬영 테크니컬 가이드입니다. 반드시 첫 문단에 관측지 천문 분석 근거를 명시하고, 이어서 기기별 최적 촬영 세팅값을 불릿으로 제공합니다.",
          },
          { role: "user", content: prompt },
        ],
      }),
